...
def ik_closed_form(x, y, z, elbow_up=False):
    """
    Closed-form IK solver for a 2-link planar arm.
    ...
    # Step 2: Reduce to 2D (XZ plane)
    r = math.hypot(x, y)   # horizontal distance
    h = z                  # vertical distance
    d = math.hypot(r, h)   # straight-line distance
    ...
    # Build 6-DOF joint array
    angles = [theta0, theta1_deg, theta2_deg, 0, 0, 0]

    # Debug print
    print("IK Solution:")
    print(f" Base:     {theta0:.2f}°")
    print(f" Shoulder: {theta1_deg:.2f}°")
    print(f" Elbow:    {theta2_deg:.2f}°")
    print(f" Config:   {'elbow-up' if elbow_up else 'elbow-down'}")
...
def main():
    ...
    move_arm_to_parts(10, 0, 10, n=4, elbow_up=False)
